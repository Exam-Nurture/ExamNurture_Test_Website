  ...
  return (
    <div className="card p-6">
      <div className="flex items-start justify-between mb-5">
        ...
        <a href="/tests" className="text-[11px] font-semibold hover:underline" style={{ color: "var(--blue)" }}>See all →</a>
      </div>
      ...
      {ROWS.map((r) => {
        ...
        return (
          ...
          >
            ...
            <div>
              <span className="text-[11px] font-bold px-2 py-1 rounded-[6px] font-mono"
                style={{ background: g.bg, color: g.fg }}>{r.percentile} %ile</span>
            </div>
            <div className="flex items-center gap-1.5 justify-end">
              <button className="w-7 h-7 rounded-[6px] flex items-center justify-center transition-all hover:bg-[var(--blue-soft)] hover:text-[var(--blue)]"
                style={{ color: "var(--ink-4)" }}>
                <Eye size={14} />
              </button>
              <Link href="/results/mock-13"
                className="text-[11px] font-semibold px-2 py-1.5 rounded-[6px] transition-all hover:bg-[var(--blue-soft)]"
                style={{ color: "var(--blue)" }}>Review →</Link>
            </div>
          </div>
        );
      })}
